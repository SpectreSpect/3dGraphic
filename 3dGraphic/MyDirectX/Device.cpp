        ...
        for (int i = 0; i < verticesCount; i += 3)
            for (int x = viewPort.left; x < viewPort.right; x++)
                for (int y = viewPort.top; y < viewPort.bottom; y++)
                {
                    ...
                    //float4 kek = *(float4*)((char*)swapChain->backBuffers[swapChain->currentBackBufferId].texture2D[x] + (y * swapChain->backBuffers->format));

                      //int point = 0;
                    //memcpy((char*)swapChain->backBuffers[swapChain->currentBackBufferId].texture2D[x] + (y * swapChain->backBuffers->format), kek, swapChain->backBuffers->format);
                }
    }
    else
        throw;
